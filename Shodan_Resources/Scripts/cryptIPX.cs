    ...
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if(!String.IsNullOrEmpty(richTextBox1.Text))
            {
                if (richTextBox1.Text.Length <= 250)
                {
                    ...
                    int counter = 0;
                    text_to_be_encrypted = richTextBox1.Text;
                    ...
                    for (int i = 0; i < text_to_be_encrypted.Length; i++)
                    {
                        ...
                        /*DISPLAY INFO*/
                        label1.Text = "Crypt meth: " + encryption_script_method.ToString() + " Generated char: of " + char_from_notcrypted_text + " + " + char_from_cryptyngscrypt + " is <" + generatedchar + "> ASCII char from crypt: " + (int)(char_from_cryptyngscrypt) + " ASCII gen: " + (int)(generatedchar);
                    }
                    encrypted_text_SB.Append(random_Char());
                    encrypted_text_SB.Append(random_Char());
                    richTextBox2.Text = encrypted_text_SB.ToString();
                }
                else
                {
                    /*can't encrypt texts longer than 250 chars*/
                    richTextBox2.Text = "can't encrypt texts longer than 250 chars";
                }
            }
            else
            {
                /*SPEAK STRING IS EMPTY*/
                richTextBox2.Text = "There is no text to be encrypted";
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if(!String.IsNullOrEmpty(richTextBox1.Text))
            {
                if (richTextBox1.Text.Length <= 250)
                {
                    text_to_be_decrypted = richTextBox1.Text;
                    ...
                    for (int i = 3; i < text_to_be_decrypted.Length - 1; i += 3)
                    {
                        ...
                        richTextBox3.Text += "Char to decrypt: " + char_from_crypted_text + " Gen char: " + generatedchar + "\n";
                        if (counter >= chosen_scrypt_string.Length)
                        {
                            counter = 0;
                        }
                    }
                    richTextBox2.Text = decrypted_text.ToString();
                    label1.Text += "Decryption key: " + encryption_script_method.ToString();
                }
                else
                {
                    /*Can't decrypt texts longer than 250 chars*/
                    richTextBox2.Text = "Can't decrypt texts longer than 250 chars";
                }
            }
            else
            {
                /*Speak error*/
                richTextBox2.Text = "There is no text to be decrypted!";
            }
        }

        private void button3_Click(object sender, EventArgs e)
        {
            richTextBox1.Text = cryptstrings[cont] + cryptstrings[cont].Length;
            cont++;
            if (cont >= cryptstrings.Length)
                cont = 0;
        }
    }
}
